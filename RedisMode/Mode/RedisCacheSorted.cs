...
{
    public class RedisCacheSorted<T>: Cache<T> where T:Entity
    {
        protected static readonly ILog Log = LogManager.GetLogger(typeof(RedisCacheString<T>).Name);
        ...
        public RedisCacheSorted(bool isCacheEnabled) : base(isCacheEnabled)
        {
            InitializeConnection();
        }
        ...
        protected override void SetStringProtected(string key, string objectToCache, TimeSpan? expiry = null)
        {
            ...
            this.RedisDatabase.SortedSetAdd(key, objectToCache,100);
        }
